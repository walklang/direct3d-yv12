...
public:
	...
	virtual bool RestoreDeviceObjects ()
	{
		...
		if ( !CreateRender2Frame ( m_lWidth / 2, m_lHeight, &m_DisplayRect ) )
		{
			return false ;
		}
...
private:
	...
	bool CreateRender2Frame ( long lWidth, long lHeight, RECT *rect )
	{
		...
		do
		{
			...
			hr = m_pDevice->CreateTexture ( lWidth, lHeight, 1, D3DUSAGE_RENDERTARGET, D3DFMT_X8R8G8B8, D3DPOOL_DEFAULT, &m_pTexture[RENDER2FRAME], NULL ) ;
	...
	bool SetImageRect ( ID3DVRInterface::GEOMETRIC_TRANSFORMATION Flip, RECT *rect )
	{
		...
		HRESULT hr = m_pVertices[RENDER2FRAME]->Lock ( 0, 0, (void**)&pVB, 0 ) ;
		if ( FAILED(hr) )
		{
			g_pDebug->Log_Error ( _T("m_pVertices->Lock failed!\nhr=%x"), hr ) ;
			return false ;
