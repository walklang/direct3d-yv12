...
bool CVR_TestDlg::CreateRender(void)
{
	...
	if ( !m_pD3DRender->CreateBackBuffer ( hWnd, IMAGE_WIDTH, IMAGE_HEIGHT ) )
	{
		return false ;
	}
	...
	bool bret = m_pD3DRender->CreateImageBuffer ( &dwImageIndex[0], IMAGE_WIDTH, IMAGE_HEIGHT, ID3DVRInterface::CS_YV12, &rect ) ;
	if ( !bret )
	{
		return false ;
	}

	bret = m_pD3DRender->CreateOverlayText ( &dwTextIndex[0], "°ëÍ¸Ã÷", &font ) ;
	if ( !bret )
	{
		return false ;
	}
	bret = m_pD3DRender->GetOverlayTextWH ( dwTextIndex[0], &lTextW[0], &lTextH[0] ) ;
	if ( !bret )
	{
		return false ;
	}
	...
	bret = m_pD3DRender->CreateImageBuffer ( &dwImageIndex[1], IMAGE_WIDTH, IMAGE_HEIGHT, ID3DVRInterface::CS_YV12, &rect ) ;
	if ( !bret )
	{
		return false ;
	}
	...
	bret = m_pD3DRender->CreateOverlayText ( &dwTextIndex[1], "²âÊÔ", &font ) ;
	if ( !bret )
	{
		return false ;
	}
	bret = m_pD3DRender->GetOverlayTextWH ( dwTextIndex[1], &lTextW[1], &lTextH[1] ) ;
	if ( !bret )
	{
		return false ;
	}
	...
	bret = m_pD3DRender->CreateImageBuffer ( &dwImageIndex[2], IMAGE_WIDTH, IMAGE_HEIGHT, ID3DVRInterface::CS_YV12, &rect ) ;
	if ( !bret )
	{
		return false ;
	}
...
LRESULT CVR_TestDlg::OnKickIdle(WPARAM wp, LPARAM lCount)
{
	...
	m_pD3DRender->UpdateImage( dwImageIndex[0], pDest ) ;
	m_pD3DRender->UpdateImage( dwImageIndex[1], pDest ) ;
	m_pD3DRender->UpdateImage( dwImageIndex[2], pDest ) ;


	m_pD3DRender->BeginRender ( false, 0 ) ;

	m_pD3DRender->DrawImage ( dwImageIndex[0], ID3DVRInterface::Not_Upper_Down_Flip ) ;
	m_pD3DRender->DrawImage ( dwImageIndex[1], ID3DVRInterface::Not_Upper_Down_Flip, &I_rect1 ) ;
	m_pD3DRender->DrawImage ( dwImageIndex[2], ID3DVRInterface::Not_Upper_Down_Flip, &I_rect2 ) ;

	m_pD3DRender->DrawOverlayText ( dwTextIndex[0], 0x60ff0000, &overlay1 ) ;
	m_pD3DRender->DrawOverlayText ( dwTextIndex[1], 0xff00ffff, &overlay2 ) ;
